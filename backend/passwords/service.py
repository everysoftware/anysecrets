from backend.auth.schemas import SUserRead
from backend.base.specification import Specification
from backend.base.types import UUID
from backend.passwords.models import Password
from backend.passwords.repositories import SearchPasswordSpec
...
class PasswordUseCases(Service):
    ...
    async def search(self, user: SUserRead, *spec: Specification) -> PasswordPage:
        page = await self.uow.passwords.get_many(SearchPasswordSpec(user_id=user.id), *spec)
